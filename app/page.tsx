  ...
  return (
    <div className="mx-auto w-full max-w-[1540px] space-y-6 pb-2">
      <section className="relative -mx-4 min-h-[520px] overflow-hidden bg-gradient-to-br from-sky-100 via-violet-100 to-pink-100 shadow-[0_22px_60px_rgba(15,23,42,.13)] sm:mx-0 sm:rounded-[32px] dark:bg-[#071020] dark:shadow-[0_24px_70px_rgba(0,0,0,.34)]">
        ...
        <div className="relative z-10 flex min-h-[520px] max-w-2xl flex-col justify-center p-7 sm:p-9 lg:p-12">
          ...
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link href="/create" className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500 px-7 py-4 font-black text-white shadow-[0_12px_32px_rgba(168,85,247,.38)] transition hover:-translate-y-0.5 hover:brightness-110">Start Creating <ArrowRight size={18} /></Link>
            <Link href="/tutorials" className="flex items-center justify-center gap-2 rounded-full border border-slate-300 bg-white/65 px-7 py-4 font-black text-slate-900 backdrop-blur-xl transition hover:bg-white dark:border-white/20 dark:bg-white/10 dark:text-white dark:hover:bg-white/20"><Play size={18} /> Watch Demo</Link>
          </div>
      ...
      <section className="grid auto-rows-fr gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatActionCard href="/dashboard" accent="bg-emerald-100/65 dark:bg-emerald-950/35" icon={<Star className="h-8 w-8 fill-white" />} label="Your Level" title="Creative Beginner" description="Level 3 · 650 / 1000 XP" action="View Progress">
        ...
        <StatActionCard href="/tutorials/paper-butterfly" accent="bg-orange-100/65 dark:bg-rose-950/35" icon={<Trophy className="h-8 w-8" />} label="Daily Challenge" title="Make a Paper Flower" description="Reward: +50 XP" action="Start Challenge" />
        <StatActionCard href="/ai-tutor" accent="bg-purple-100/65 dark:bg-purple-950/35" icon={<Bot className="h-8 w-8" />} label="AI Tutor" title="Ask for craft help" description="Get tips, stitch guides and creative ideas." action="Chat with AI" />
        <StatActionCard href="/community" accent="bg-cyan-100/65 dark:bg-cyan-950/35" icon={<Users className="h-8 w-8" />} label="Community" title="12.5K Creators" description="Share, inspire and grow together." action="Explore Community" />
      </section>

      <section>
        <div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Explore Categories</h2><Link href="/explore" className="font-bold text-purple-600 transition hover:text-pink-500 dark:text-purple-300">View All categories →</Link></div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {categories.map((item) => <Link href={item.href} key={item.name} className={`rounded-2xl bg-gradient-to-br ${item.color} p-4 text-white shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-5`}><div className="text-3xl">{item.emoji}</div><h3 className="mt-3 font-extrabold">{item.name}</h3><p className="text-sm text-white/80">{item.count}</p></Link>)}
        </div>
      </section>

      <section>
        <div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Trending Tutorials</h2><Link href="/tutorials" className="font-bold text-purple-600 transition hover:text-pink-500 dark:text-purple-300">View All tutorials →</Link></div>
        <div className="grid gap-4 md:grid-cols-3">
          {tutorials.map((item) => <Link href={item.href} key={item.title} className="overflow-hidden rounded-[28px] border border-slate-200 bg-white/80 p-4 shadow-lg backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-xl dark:border-white/10 dark:bg-white/10"><div className="mb-4 flex h-36 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-400 via-purple-500 to-pink-500 text-5xl">{item.emoji}</div><p className="text-sm font-bold text-purple-600 dark:text-purple-300">{item.category}</p><h3 className="mt-1 text-xl font-black text-slate-900 dark:text-white">{item.title}</h3><div className="mt-4 flex justify-between text-sm text-slate-600 dark:text-slate-300"><span>{item.duration}</span><span>+{item.xp} XP</span></div></Link>)}
        </div>
      </section>
    </div>
  );
